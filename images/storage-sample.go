...
func runStorage() {
	...
	basket, err := service.Buckets.Get(bucketName).Do()
	if basket != nil {
		log.Println("WE'VE FOUND the Bucket: %v", basket)
		if err != nil {
			log.Println("BUT WE ALSO GOT SOME BAD ERROR DU NO WHY :(: %v", err)
		}
		// List all buckets in a project
		result, _ := service.Buckets.List("lestgo-1145").Do()

		log.Println("BUBUBUCKETS:")
	...
	if _, err := service.Buckets.Get(bucketName).Do(); err == nil {
		...
		// Create a bucket.
		if res, err := service.Buckets.Insert("lestgo-1145", &storage.Bucket{Name: bucketName}).Do(); err == nil {
	...
	// List all buckets in a project.
	if res, err := service.Buckets.List("lestgo-1145").Do(); err == nil {
		fmt.Println("Buckets:")
